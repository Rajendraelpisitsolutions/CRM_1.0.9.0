...
function Deals({
  ...
  // Contacts this note is being shared with while editing (string ids). Seeded from the note's
  // current shared contacts so old notes can be linked to their relations via the update UI.
  const [editNoteContacts, setEditNoteContacts] = useState(() => new Set());
  // Validation message shown in the edit checklist when no contact is selected.
  ...
  // Enter edit mode for a note, seeding the share checklist with its current shared contacts.
  const startEditNote = (note) => {
    setEditingNoteId(note.id || note.Id);
    setEditingNoteText(note.description || note.Description || "");
    const shared = note.sharedWithContacts || note.SharedWithContacts || [];
    setEditNoteContacts(new Set(shared.map((c) => String(c.id ?? c.Id)).filter((x) => x && x !== "undefined")));
    setEditNoteError("");
  };
  ...
  const cancelEditNote = () => {
    ...
    setEditingNoteText("");
    setEditNoteContacts(new Set());
    setEditNoteError("");
  };

  const toggleEditNoteContact = (id) => {
    setEditNoteError("");
    setEditNoteContacts((prev) => {
      const next = new Set(prev);
      const key = String(id);
  ...
  const handleUpdateNote = async (note) => {
    // A deal note must be shared with at least one contact — block the save otherwise.
    // (Only enforced when the deal actually has contacts to pick from.)
    if (noteEligibleContacts.length > 0 && editNoteContacts.size === 0) {
      setEditNoteError("Select at least one contact before saving.");
      if (onToast) onToast("Select at least one contact to save this note.", "error");
      return;
    }
    try {
      ...
      const payload = {
        ...
        // Ticked contacts in the edit checklist → new NoteTargets references (backend is idempotent).
        mirrorToContactIds: Array.from(editNoteContacts).map(Number).filter((n) => Number.isFinite(n) && n > 0),
      };
      ...
      // Reload from the server so the note now carries any newly-shared contact chips.
      try {
        const dealId = getDealField(selectedDealDetails, "Id") || selectedDealDetails?.dealId || selectedDealDetails?.id;
        if (dealId) fetchDealNotes(dealId);
      } catch (e) { /* ignore */ }
    ...
    >
      ...
      <div className="relative">
        <button
          ...
          className="w-full flex items-center justify-between rounded-lg border border-gray-300 px-3 py-2 text-sm bg-white hover:bg-gray-50 focus:ring-2 focus:ring-slate-500 focus:outline-none"
        >
  ...
  // Compact share checklist shown while EDITING a note — tick the deal's contacts this note
  // should also appear under. Ticking creates NoteTargets references on save (see handleUpdateNote).
  const editShareChecklist = (
    <div className={`mt-3 rounded-xl border p-3 ${editNoteError ? "border-red-300 bg-red-50" : "border-gray-200 bg-gray-50"}`}>
      <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">
        Share with contact(s) <span className="text-red-500">*</span>
      </p>
      {noteEligibleContacts.length === 0 ? (
        <p className="text-xs text-gray-400">No related contacts for this deal.</p>
      ) : (
        <div className="flex flex-col gap-1.5 max-h-40 overflow-y-auto">
          {noteEligibleContacts.map((c) => (
            <label key={c.id} className="flex items-center gap-2 text-sm cursor-pointer">
              <input
                type="checkbox"
                checked={editNoteContacts.has(String(c.id))}
                onChange={() => toggleEditNoteContact(c.id)}
                className="w-4 h-4"
              />
              <span>{c.name}</span>
            </label>
      ...
      {editNoteError && <p className="text-xs text-red-600 mt-2">{editNoteError}</p>}
    </div>
  );
    ...
    <div className="flex flex-col h-full font-[poppins,sans-serif] bg-gray-50">
      ...
      <div className="relative flex items-stretch overscroll-scroll gap-4 p-4 overflow-x-auto flex-1 visible-scrollbar" style={{ overflowY: "hidden" }}>
        ...
        {orderedStages.map((stage) => (
          <div key={stage} className="bg-white rounded-xl w-[340px] sm:w-[380px] flex flex-col shadow-sm border border-gray-200 flex-shrink-0 h-full overflow-hidden">
            <div className="flex justify-between items-center px-4 py-3 border-b border-gray-200 gap-2 shrink-0" style={{ backgroundColor: stageHeaderColors[stage] || undefined }}>
              <h3 className="font-[poppins,sans-serif] text-gray-700 text-sm font-semibold truncate">{stage}</h3>
                ...
                {dealsByStage[stage] && dealsByStage[stage].length > 0 ? (
                  dealsByStage[stage]
                    ...
                    .map((deal) => {
                      ...
                      return (
                        <div key={deal.dealId || deal.id} className="group relative bg-white rounded-xl p-3 shadow-sm hover:shadow-md border border-gray-200 hover:border-gray-300 transition-all duration-200 overflow-hidden min-h-[200px] flex flex-col">
                          <button className="absolute top-2 right-2 w-7 h-7 rounded-lg bg-gray-50 hover:bg-gray-100 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 z-10 focus:ring-2 focus:ring-slate-500 focus:outline-none" aria-label="Show more options" onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === (deal.dealId || deal.id) ? null : (deal.dealId || deal.id)); }}>
                          ...
                          {openMenuId === (deal.dealId || deal.id) && (
                            <div ref={dropdownRef} className="absolute top-9 right-2 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-20 min-w-36 animate-in fade-in zoom-in-95 duration-200">
                              <button onClick={() => { handleShowDetails(deal.dealId || deal.id); setOpenMenuId(null); }} className="w-full px-3 py-1.5 text-left text-xs text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors flex items-center gap-2 focus:ring-2 focus:ring-slate-500 focus:outline-none">
                          ...
                          <div className="relative flex h-full flex-col">
                            {/* Deal Name (left) + Priority badge (right / opposite the name) */}
                            <div className="flex justify-between items-start gap-2 mb-2 pr-7">
                              ...
                              }} className="flex-1 min-w-0 text-sm font-semibold text-slate-900 cursor-pointer hover:text-slate-950 transition-colors duration-200 line-clamp-2 leading-5 hover:underline">{deal.name}</a>
                              {(() => {
                                const pr = String(deal.priority ?? deal.Priority ?? "").trim();
                                if (!pr) return null;
                                const key = pr.toLowerCase();
                                const style = {
                                  high: { pill: "bg-red-100 text-red-700 border-red-200", dot: "bg-red-500" },
                                  medium: { pill: "bg-amber-100 text-amber-700 border-amber-200", dot: "bg-amber-500" },
                                  low: { pill: "bg-emerald-100 text-emerald-700 border-emerald-200", dot: "bg-emerald-500" },
                                }[key] || { pill: "bg-slate-100 text-slate-600 border-slate-200", dot: "bg-slate-400" };
                                return (
                                  <span className={`shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-bold uppercase tracking-wide ${style.pill}`}>
                  ...
                  <div className="flex flex-col items-center justify-center text-center py-8 px-3">
                    ...
                    <p className="text-sm font-medium text-gray-600">No deals</p>
                    <p className="text-xs text-gray-400 mt-0.5">Nothing in this stage yet</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="p-5 border-b bg-gray-50">
                {!showAddNoteBox ? (
                  <button
                    ...
                    className="w-full py-3 rounded-lg border border-dashed border-gray-300 text-gray-600 hover:bg-gray-50 hover:border-gray-400 transition-all text-sm font-medium"
                  >
                    + New Note
                  </button>
                ) : (
                  <div className="space-y-3 animate-in fade-in duration-200">
                    ...
                    <AutoGrowTextarea
                      ...
                      className="w-full rounded-lg border border-gray-300 focus:ring-2 focus:ring-slate-500 focus:border-transparent outline-none px-3 py-2 text-sm"
                    />
        ...
        <p className="text-xs text-red-600 pt-1">{noteDestinationError}</p>
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        className="flex-1 py-2 rounded-lg bg-slate-800 hover:bg-slate-900 text-white text-sm font-medium"
                      >
                        Save Note
                      </button>
                      <button
              ...
              {noteEligibleContacts.length > 0 && (
                <div className="px-5 pt-3 pb-2 flex items-center gap-2 flex-wrap border-b">
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-medium transition ${dealNotesContactFilter == null ? "bg-slate-800 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                  >
                    All
                  </button>
                  {noteEligibleContacts.map((c) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-xs font-medium transition ${String(dealNotesContactFilter) === String(c.id) ? "bg-slate-800 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                    >
                      {c.name}
                    </button>
                  ))}
                        ...
                        {editingNoteId === noteId ? (
                          <>
                            ...
                            <div className="flex gap-2 mt-3">
                              <button
                                ...
                                className="px-4 py-2 rounded-xl bg-slate-800 text-white text-sm">Save</button>
                              <button
                          ...
                          <>
                            ...
                            >
                              ...
                              {noteText?.length > 120 && (
                                <button type="button" className="mt-2 text-xs text-slate-600 hover:text-slate-800 font-medium"> {expandedNotes[noteId] ? "Show Less" : "Read More"} </button>
                              )}
                            </div>
            ...
            <div className="bg-white shadow-2xl w-[60%] h-full flex flex-col overflow-hidden animate-in slide-in-from-right duration-300 text-sm pointer-events-auto">
              <div className="flex flex-col gap-4 p-4 border-b border-gray-200/80 bg-white/80 backdrop-blur-sm">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    className="px-2 sm:px-4 py-2 rounded-lg border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 text-sm font-medium transition-all duration-200 flex items-center justify-center gap-2 focus:ring-2 focus:ring-slate-500 focus:outline-none"
              ...
              <div className="relative flex-1 overflow-y-auto p-8">
                <form className="space-y-8" onSubmit={async (e) => {
                  ...
                  if (!String(selectedDealDetails.priority || selectedDealDetails.Priority || "").trim()) {
                    if (onToast) onToast("Priority is required. Please select High, Medium or Low.", "error");
                    return;
                  }
                  ...
                  <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                    <div className="flex items-center gap-2 mb-4">
                      <div className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center"><svg className="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
                      <label className="font-semibold text-gray-900 text-md">Deal Stage</label>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      {dealStages.map((stage) => (
                        <button key={stage} type="button" className={["relative inline-flex items-center font-medium px-4 py-2.5 rounded-lg cursor-pointer transition-all duration-200 text-sm", selectedDealDetails?.dealStage === stage ? "bg-slate-800 text-white ring-2 ring-slate-300" : "bg-gray-50 text-gray-700 hover:bg-gray-100 border border-gray-200 hover:border-gray-300"].join(" ")} onClick={() => setSelectedDealDetails({ ...selectedDealDetails, dealStage: stage })}>
                  ...
                  {detailsLoading && <div className="flex items-center justify-center py-12"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-slate-700"></div></div>}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            return (
                              <>
                                <input
                                  ...
                                  className="w-full rounded-xl px-4 py-3.5 pr-10 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150"
                                />
                        ...
                        <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Deal Type</label>
                            <div className="relative">
                              <select
                                className="w-full rounded-xl pl-4 pr-10 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150 appearance-none cursor-pointer"
                          ...
                          <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Priority <span className="text-red-500">*</span></label><div className="relative"><select required className="w-full rounded-xl pl-4 pr-10 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150 appearance-none cursor-pointer" value={selectedDealDetails.priority || selectedDealDetails.Priority || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, priority: e.target.value })}><option value="High">High</option><option value="Medium">Medium</option><option value="Low">Low</option></select><svg className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></div></div>
                          {/* <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Deal Pipeline</label><input type="text" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="Enter deal pipeline" value={selectedDealDetails.dealPipeline || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, dealPipeline: e.target.value })} /></div> */}
                          <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Deal Pipeline</label><div className="relative"><select className="w-full rounded-xl pl-4 pr-10 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150 appearance-none cursor-pointer" value={selectedDealDetails?.dealPipeline || selectedDealDetails?.DealPipeline || "default pipeline"} onChange={(e) => setSelectedDealDetails({ ...selectedDealDetails, dealPipeline: e.target.value })}><option value="default pipeline">Default Pipeline</option><option value="Hardware Product Sale">Hardware Product Sale</option><option value="Software Product Sale">Software Product Sale</option><option value="Software/Hardware Pipeline">Software/Hardware Pipeline</option></select><svg className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></div></div>
                          <div className="md:col-span-2 bg-gray-50 rounded-xl p-4 border border-gray-200 space-y-3">
                            <div className="flex items-center justify-between"><div className="flex items-center gap-2"><label className="font-semibold text-gray-800 text-sm">Products</label>{selectedProducts && selectedProducts.length > 0 && (<span className="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full font-medium">{selectedProducts.length} product(s)</span>)}</div></div>
                            <div className="bg-white p-3 rounded-lg border border-gray-200 space-y-2"><label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Add Product</label><div className="flex gap-2"><select id="slideinProductSelect" className="flex-1 rounded-lg px-3 py-2 border border-gray-300 text-sm focus:ring-2 focus:ring-slate-500 focus:border-transparent outline-none transition-all" defaultValue=""><option value="">Select product to add</option>{(Array.isArray(productsList) ? productsList : []).map((p) => { const name = p?.name || p?.Name || ""; const isSelected = Array.isArray(selectedProducts) && selectedProducts.some(sp => sp.name === name); return (<option key={name} value={name} disabled={isSelected}>{name} {isSelected ? "(added)" : ""}</option>); })}</select><button type="button" onClick={() => { const select = document.getElementById("slideinProductSelect"); const name = select.value; if (!name || (Array.isArray(selectedProducts) && selectedProducts.some(sp => sp.name === name))) { return; } const prod = (Array.isArray(productsList) ? productsList : []).find(p => (p?.name || p?.Name) === name); const cat = prod ? (prod?.category || prod?.Category || "") : ""; setSelectedProducts([...(selectedProducts || []), { name, category: cat }]); select.value = ""; }} className="px-4 py-2 bg-slate-800 hover:bg-slate-900 text-white text-sm rounded-lg transition-colors font-medium">+ Add</button></div></div>
                            {selectedProducts && selectedProducts.length > 0 && (<div className="bg-white p-3 rounded-lg border border-gray-200 space-y-2"><label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Products in Deal</label><div className="space-y-2 max-h-48 overflow-y-auto">{selectedProducts.map((prod, idx) => (<div key={idx} className="flex items-center justify-between p-2.5 bg-gray-50 rounded-lg border border-gray-200 group hover:border-gray-300 transition-all"><div className="flex-1"><p className="text-sm font-medium text-gray-800">{prod.name}</p><p className="text-xs text-gray-500">{prod.category || "Uncategorized"}</p></div><button type="button" onClick={() => { setSelectedProducts((selectedProducts || []).filter((_, i) => i !== idx)); }} className="ml-2 px-3 py-1 text-xs bg-red-500 hover:bg-red-600 text-white rounded font-medium opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Remove</button></div>))}</div></div>)}
                            {(!selectedProducts || selectedProducts.length === 0) && (<div className="text-center py-3 bg-white rounded-lg border border-dashed border-gray-300"><p className="text-xs text-gray-500 font-medium">No products added yet</p></div>)}
                          </div>
                        </div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Deal Source</label><input type="text" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="Enter deal source" value={selectedDealDetails.source || selectedDealDetails.dealSource || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, source: e.target.value })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Territory</label><input type="text" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="Enter territory" value={selectedDealDetails.territory || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, territory: e.target.value })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Deal Age (Days)</label><input type="number" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="Enter deal age in days" value={selectedDealDetails.ageInDays || selectedDealDetails.dealAgeInDays || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, ageInDays: e.target.value })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Sales Owner</label><input type="text" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="Enter sales owner" value={selectedDealDetails.salesOwner || selectedDealDetails.dealSalesOwner || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, salesOwner: e.target.value })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Campaign</label><input type="text" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="Enter campaign" value={selectedDealDetails.campaign || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, campaign: e.target.value })} /></div>
                        <div className="md:col-span-2 flex items-center gap-3 mt-4 mb-1"><span className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">Financials &amp; Forecast</span><span className="flex-1 h-px bg-gray-100" /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Probability (%)</label><input type="number" min="0" max="100" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="0-100" value={selectedDealDetails.probability || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, probability: e.target.value })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Expected Close Date</label><input type="date" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" value={formatDateOnly(selectedDealDetails.expectedCloseDate || "")} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, expectedCloseDate: e.target.value ? dateToISOLocal(e.target.value) : "" })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Closed Date</label><input type="date" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" value={formatDateOnly(selectedDealDetails.closedDate || "")} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, closedDate: e.target.value ? dateToISOLocal(e.target.value) : "" })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Payment Status</label><div className="relative"><select className="w-full rounded-xl pl-4 pr-10 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150 appearance-none cursor-pointer" value={selectedDealDetails.paymentStatus || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, paymentStatus: e.target.value })}><option value="">Select Status</option><option value="Pending">Pending</option><option value="Partial">Partial</option><option value="Completed">Completed</option><option value="Failed">Failed</option></select><svg className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></div></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Lost Reason {selectedDealDetails.dealStage === "Lost" && <span className="text-red-500">*</span>}</label><input type="text" className={`w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150 ${selectedDealDetails.dealStage === "Lost" && !String(selectedDealDetails.lostReason || "").trim() ? "border-red-300 focus:border-red-500" : "border-gray-200 focus:border-slate-500"}`} placeholder="Enter reason if deal was lost" value={selectedDealDetails.lostReason || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, lostReason: e.target.value })} />{selectedDealDetails.dealStage === "Lost" && !String(selectedDealDetails.lostReason || "").trim() && <p className="text-red-500 text-xs">Lost Reason is required for Lost deals.</p>}</div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Won Reasons {selectedDealDetails.dealStage === "Won" && <span className="text-red-500">*</span>}</label><input type="text" className={`w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150 ${selectedDealDetails.dealStage === "Won" && !String(selectedDealDetails.wonReasons || "").trim() ? "border-red-300 focus:border-red-500" : "border-gray-200 focus:border-slate-500"}`} placeholder="Enter reason if deal was won" value={selectedDealDetails.wonReasons || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, wonReasons: e.target.value })} />{selectedDealDetails.dealStage === "Won" && !String(selectedDealDetails.wonReasons || "").trim() && <p className="text-red-500 text-xs">Won Reasons is required for Won deals.</p>}</div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Forecast Category</label><div className="relative"><select className="w-full rounded-xl pl-4 pr-10 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150 appearance-none cursor-pointer" value={selectedDealDetails.forecastCategory || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, forecastCategory: e.target.value })}><option value="">Select Category</option><option value="Pipeline">Pipeline</option><option value="BestCase">Best Case</option><option value="Likely">Likely</option><option value="Commit">Commit</option></select><svg className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></div></div>
                        <div className="md:col-span-2 flex items-center gap-3 mt-4 mb-1"><span className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">Meta</span><span className="flex-1 h-px bg-gray-100" /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Enquiry Number</label><input type="text" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="Enter enquiry number" value={selectedDealDetails.enquiryNumber || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, enquiryNumber: e.target.value })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Import ID</label><input type="text" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="Enter import ID" value={selectedDealDetails.importId || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, importId: e.target.value })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Tags</label><input type="text" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="Enter tags" value={selectedDealDetails.tags || selectedDealDetails.dealTags || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, tags: e.target.value })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Created By</label><input type="text" disabled className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150 opacity-60 cursor-not-allowed" value={selectedDealDetails?.createdBy || selectedDealDetails?.CreatedBy || "Unknown"} /></div>
                        ...
                        <div className="flex flex-col gap-2 md:col-span-2"><label className="font-normal text-gray-700 text-sm">Recent Note</label><AutoGrowTextarea minRows={4} className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" placeholder="Enter recent note" value={selectedDealDetails.recentNote || selectedDealDetails.dealRecentNote || ""} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, recentNote: e.target.value })} /></div>
                        <div className="flex flex-col gap-2"><label className="font-normal text-gray-700 text-sm">Created At</label><input type="date" className="w-full rounded-xl px-4 py-3.5 text-gray-800 bg-gray-50 border-2 border-gray-200 focus:border-slate-500 focus:bg-white focus:ring-4 focus:ring-slate-500/10 outline-none transition-all duration-150" value={formatDateOnly(getDealField(selectedDealDetails, 'CreatedAt') || "")} onChange={e => setSelectedDealDetails({ ...selectedDealDetails, DealCreatedAt: e.target.value ? dateToISOLocal(e.target.value) : "" })} /></div>
                  ...
                  <div className="flex justify-between items-center gap-3 pt-6 border-t-2 border-gray-200 mt-8 bg-white/50 backdrop-blur-sm rounded-xl p-6 -mx-2">
                    {/* Delete — Admin only */}
                    {isAdmin && !isManager && (
                      <button type="button" className="px-5 py-3 rounded-lg border border-red-200 bg-white hover:bg-red-50 hover:border-red-300 text-red-600 font-medium transition-all duration-200 flex items-center gap-2 focus:ring-2 focus:ring-red-500 focus:outline-none" onClick={() => { if (selectedDealDetails) { setDeleteDealId(getDealField(selectedDealDetails, 'Id') ?? null); setDeleteDealName(getDealField(selectedDealDetails, 'Name') || getDealField(selectedDealDetails, 'Id') || ""); setShowDeleteModal(true); } }}>
                    ...
                    {canEditDeal(selectedDealDetails) ? (
                      <button type="submit" className="px-6 py-3 rounded-lg bg-slate-800 hover:bg-slate-900 text-white font-medium shadow-sm transition-all duration-200 flex items-center gap-2 ml-auto focus:ring-2 focus:ring-slate-500 focus:outline-none">
                  ...
                  <div className="absolute top-0 right-0 z-40 h-full w-[30%] bg-white border-l border-gray-200 shadow-2xl overflow-hidden animate-in slide-in-from-right duration-300">
                    ...
                    <div className="p-6 h-full overflow-y-auto space-y-4">
                      <div className="flex justify-between items-center gap-3">
                        ...
                        <button type="button" onClick={() => setShowAddNoteBox(!showAddNoteBox)} className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-900 text-white text-sm transition">
                        ...
                        <div className="space-y-3 rounded-2xl border border-gray-200 bg-gray-50 p-4">
                          ...
                          <div className="flex gap-2">
                            <button
                              type="button"
                              onClick={handleSaveDealNote}
                              className="flex-1 py-2 rounded-xl bg-slate-800 hover:bg-slate-900 text-white text-sm"
                            >
                              Save Note
                            </button>
                            <button
                              ...
                              <div className="flex justify-between gap-3">
                                <div className="space-y-2">
                                  {editingNoteId === (note.id || note.Id) ? (
                                    <>
                                      <AutoGrowTextarea minRows={3} value={editingNoteText} onChange={(e) => setEditingNoteText(e.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:border-slate-500 outline-none" />
                                      {editShareChecklist}
                                    </>
                                  ) : (
                                ...
                                <div className="flex flex-col gap-2 items-end">
                                  {editingNoteId === (note.id || note.Id) ? (
                                    <>
                                      <button type="button" onClick={() => handleUpdateNote(note)} className="text-xs px-3 py-1 rounded-lg bg-slate-800 text-white">Save</button>
                                      <button type="button" onClick={cancelEditNote} className="text-xs px-3 py-1 rounded-lg bg-gray-200 text-gray-700">Cancel</button>
                                    </>
                                  ) : (
                                    <>
                                      <button type="button" onClick={() => startEditNote(note)} className="text-xs px-3 py-1 rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200">Edit</button>
